...
func RunOperator(ctx *controllercmd.ControllerContext) error {
	...
	kubeInformersNamespaced := informers.NewSharedInformerFactoryWithOptions(kubeClient, resync, informers.WithNamespace(targetName))
	...
	routeInformersNamespaced := routeinformer.NewSharedInformerFactoryWithOptions(routeClient, resync, routeinformer.WithNamespace(targetName), routeinformer.WithTweakListOptions(singleNameListOptions(targetName)))
	...
	if err := resourceSyncer.SyncConfigMap(resourcesynccontroller.ResourceLocation{Namespace: machineConfigNamespace, Name: targetName}, resourcesynccontroller.ResourceLocation{Namespace: targetName, Name: oauthMetadataName}); err != nil {
		return err
	}
	if err := resourceSyncer.SyncSecret(resourcesynccontroller.ResourceLocation{Namespace: targetName, Name: routerCertsLocalName}, resourcesynccontroller.ResourceLocation{Namespace: machineConfigNamespace, Name: routerCertsSharedName}); err != nil {
		return err
	}
	...
	clusterOperatorStatus := status.NewClusterOperatorStatusController(clusterOperatorName, []configv1.ObjectReference{{Group: operatorv1.GroupName, Resource: "authentications", Name: globalConfigName}, {Group: configv1.GroupName, Resource: "authentications", Name: globalConfigName}, {Group: configv1.GroupName, Resource: "infrastructures", Name: globalConfigName}, {Group: configv1.GroupName, Resource: "oauths", Name: globalConfigName}, {Resource: "namespaces", Name: userConfigNamespace}, {Resource: "namespaces", Name: machineConfigNamespace}, {Resource: "namespaces", Name: targetName}, {Resource: "namespaces", Name: targetNameOperator}}, configClient.ConfigV1(), configInformers.Config().V1().ClusterOperators(), operatorClient, versionGetter, ctx.EventRecorder)
